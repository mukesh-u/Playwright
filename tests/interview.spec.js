...
test("test", async ({ page }) => {
  await page.goto(
    "https://indiarunning-organizer-dashboard-staging.bombayrunning.com/login"
  );
  await page.getByLabel("Please enter your email").click();
  await page
    .getByLabel("Please enter your email")
    .fill("mukeshupadhyay0612@gmail.com");
  ...
  await page.locator("#event_start_date").fill("2025-04-25");
  ...
  await page
    .locator(".ql-editor")
    .fill("Hritik roshan is our featured guest for this event");
  await page.getByText("Save and Proceed").click();

  // next page
  await page.locator('//input[@type="radio" and @id="Running"]').click();
  await page.selectOption("#tickets\\.0\\.distance", {
    label: "10 Kilometers",
  });
  await page
    .locator('//input[@id="Name of the Category_text"]')
    .fill("half marathon");
  await page.locator('//input[@id="Base Price"]').fill("1000");
  await page.locator('//input[@id="Age Limit for Registration"]').fill("21");
  await page.locator('[id="tickets\\.0\\.reg_start_date"]').fill("2025-04-25");
  await page.fill('input[name="tickets\\.0\\.reg_start_time"]', "10:00");
  await page.getByRole("button", { name: "Create Ticket" }).click();

  // create form
  await page.getByRole("button", { name: "Proceed to create form" }).click();
